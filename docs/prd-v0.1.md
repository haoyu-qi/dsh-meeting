...
>
> 配套交互设计见 [interaction-v0.1.md](interaction-v0.1.md)：一条核心闭环 + 两个辅助入口，仅 3 个核心界面。本文已同步其影响需求范围的增量决策（Meeting UI 形态、协作视图、`topic` 当前讨论、Agent 介入方式与结果联动等）。
...
结束交互（V0.1）：普通成员退出需确认；Host 结束弹出确认框（列明将保留的转写/决策/Agent 任务与执行记录）；结束后不跳转"会议记录"，停留在当前 Project，Agent 对话中生成协作总结卡（主题/决策/完成/待处理 + [继续让 Agent 处理]，P1）——**会议结束，开发上下文没有结束**。
...
| Meeting UI | 顶部/右上角「协作」入口展开右侧 Panel（280~320px）；仅 **3 个核心界面**：协作大厅（附近房间 + 创建）、协作侧边栏（成员/语音/共享状态/最近转写/Agent 状态/当前讨论）、共享屏幕视图（中心区协作视图）。视觉优先级：共享内容 > Agent > 参会人，不做成员宫格 |
...
- 共享屏幕查看：其他成员点 [查看共享] 后在 Harness 中心区域临时进入「协作视图」（非新窗口，可退出查看）；共享开始即自动触发 Agent 屏幕上下文同步（无"截图→上传→分析"链路）；
...
交互设计新增字段：`topic`（**当前讨论 / Active Context**）——超短主题描述 + AI 识别的相关文件（如 `{"text": "登录接口重复请求", "relatedFiles": ["Login.tsx"]}`），随讨论自动更新，让所有成员与 Agent 随时知道"现在在解决什么"。V0.1 建议纳入（更新策略见 §17）。
...
对外展示使用**人类可读短语**：`分析中 / 读取代码 / 修改代码 / 运行测试 / 等待确认 / 已完成`（如 `● 正在修改 Login.tsx`、`● 正在运行 npm test`），全体成员在协作侧边栏可见，不要求展示完整 Agent Chain。
...
### 11.6 Agent 介入方式与结果联动（交互设计）

三种介入方式：

1. **Harness 对话直接询问（默认）**：Agent 自动携带 Meeting Context；
2. **语音唤起**：识别"Agent，…"唤起词 → 显示"🤖 Agent 正在分析..."，结果仍显示在原生 Agent 对话区，**不新开 Agent Chat**；
3. **会议自动建议（克制）**：决策/任务卡片，操作为 `[记录] [让 Agent 实现] ×`。

**「让 Agent 实现」不在插件内执行代码**：自动向 Harness Agent 发起任务（如"根据当前会议上下文，实现刚才确认的验证码 Mock 方案"），随后完全回归原生流程（Plan → 权限确认 → 修改 → Terminal → Diff）。完成后协作面板出现结果卡（修改文件数 / 测试结果 / +diff 行数），`[查看结果]` 跳转 Harness 原生 Diff / Agent Result，插件不重做 Diff 页面。
...
**交互验收口径（三大关键瞬间）**：① 打开 Harness 即可看到附近团队正在讨论什么；② 他人共享代码后，一句"Agent，看一下这里"，Agent 能正确理解；③ 一句"按刚才方案改"，Agent 无需重复 Prompt 即开始处理。三个交互成立，产品方向即成立。
...
11. **语音唤起词检测**："Agent，…"关键词识别的归属（本地 VAD/关键词检测 vs 云端 ASR 前置过滤）与误触率控制；
12. **`topic`（当前讨论）生成与更新策略**：规则抽取 vs LLM 摘要，更新节流频率，避免频繁跳变；
13. **「让 Agent 实现」的执行归属**：由点击者本机 Agent 执行、状态经 DataChannel 广播——多人同时触发时的冲突处理（队列 / 锁 / 仅 Host 可触发）需定案。
