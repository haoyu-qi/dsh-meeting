window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = `
      ...
      @media(max-width:900px){body.dshm-native-active div:has(> [data-shell-overlay])>div:nth-child(2){display:none}.dshm-native-shell{right:0}.dshm-roomLayout{grid-template-columns:1fr}.dshm-rail{display:none}.dshm-status{display:none}}
      ...
      @container(max-width:520px){.dshm-formGrid{grid-template-columns:1fr}.dshm-roomLayout{grid-template-columns:1fr}.dshm-rail{display:none}.dshm-topic{display:none}.dshm-control{min-width:0;flex:1}}
    ...
    function MeetingWorkspace({ onClose }) {
      ...
      useEffect(() => {
        ...
        const connect = () => {
          ...
          socket.addEventListener("open", () => setConnection("ready"));
      ...
      useEffect(() => () => {
        ...
        for (const { stream } of localTracksRef.current.values()) for (const track of stream.getTracks()) track.stop();
      ...
      const toggleAudio = useCallback(async () => {
        ...
        const issue = mediaIssue("getUserMedia", "麦克风");
      ...
      }, [addTrackToPeers, publishMedia, toast]);
      ...
      const toggleCamera = useCallback(async () => {
        ...
        const issue = mediaIssue("getUserMedia", "摄像头");
        if (issue) return toast(issue, true);
        try {
      ...
      }, [addTrackToPeers, publishMedia, removeLocalTrack, toast]);
      ...
      const toggleScreen = useCallback(async () => {
        ...
        const issue = mediaIssue("getDisplayMedia", "屏幕共享");
        if (issue) return toast(issue, true);
        try {
          ...
          !room ? h("div", { className: "dshm-lobby" },
            ...
            h("aside", { className: "dshm-rail" },
              ...
              railTab === "context" && h("form", { className: "dshm-contextForm", onSubmit: submitContext },
                h("textarea", { value: contextText, onChange: (event) => setContextText(event.target.value), maxLength: 500, placeholder: "记录一条讨论或决策" }),
                h("button", { className: "dshm-button", "data-primary": true, type: "submit" }, "写入会议上下文"),
              ),
            ),
          ),
        ),
